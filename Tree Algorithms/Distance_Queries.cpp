...
void dfs(const vector<vector<int>> &adj, vector<int> &path, vector<int> &h, vector<int> &idx, int node, int parent, int l){
	idx[node] = path.size();
	...
	for(const int &to: adj[node]){
		...
		dfs(adj, path, h, idx, to, node, l+1);
		path.push_back(node);
		h.push_back(l);
	}
}
...
void solve(){
	...
	while(q--){
		...
		cout << h[idx[u]] + h[idx[v]] - 2*h[idx[lca]] << endl;
	}
}
