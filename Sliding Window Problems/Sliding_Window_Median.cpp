...
// comprime las coordenadas
// mete k elementos en el bit, encuentra el primer valor tal que pos <= x;
...
int mean(){
	int i=0; int bit = 1<<20; int sum = 0;
	for(;bit;bit>>=1){
		if(i+bit>n*2) continue;
		if(BIT[i+bit]+sum< ((x+1)>>1)){ i+=bit; sum+=BIT[i]; }
	}
	return i+1;
}
